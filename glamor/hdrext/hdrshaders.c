...
#include "bt2020-linearnorm-to-bt2020-linear-fragment-shader.glsl.h"
#include "bt2020-linearnorm-to-bt2020-linear-vertex-shader.glsl.h"
#include "bt2020-pq-to-bt2020-linear-fragment-shader.glsl.h"
#include "bt2020-pq-to-bt2020-linear-vertex-shader.glsl.h"
...
void InitializeScreenShaders(ScreenPtr pScreen){
  ...
  /* BT2020 linear normalized to unnormalized -  it just *1000 for pixel values */
  {
    GLuint fs = glamor_compile_glsl_prog(GL_FRAGMENT_SHADER,(char *)bt2020_linearnorm_to_bt2020_linear_fragment_shader_glsl);
    GLuint vs= glamor_compile_glsl_prog(GL_VERTEX_SHADER,(char *)bt2020_linearnorm_to_bt2020_linear_vertex_shader_glsl);

    priv->BT2020_linearnorm_to_linear = glCreateProgram();
    glAttachShader(priv->BT2020_linearnorm_to_linear, vs);
    glAttachShader(priv->BT2020_linearnorm_to_linear, fs);
    glDeleteShader(vs);
    glDeleteShader(fs);

    glamor_link_glsl_prog(pScreen,priv->BT2020_linearnorm_to_linear,"bt2020_linearnorm_to_bt2020_linear");

    glamor_program *prog =  &priv->BT2020_linearnorm_to_linear_glamor;


    prog->locations = glamor_priv->copy_area_prog.locations;
    prog->prim_use = glamor_priv->copy_area_prog.prim_use;
    prog->prog = priv->BT2020_linearnorm_to_linear;
    fill_glamor_prog_variables(prog);

  }


  /* BT2020 PQ to bt2020 linear (this one non normalized) */
  {
    GLuint fs = glamor_compile_glsl_prog(GL_FRAGMENT_SHADER,(char *)bt2020_pq_to_bt2020_linear_fragment_shader_glsl);
    GLuint vs= glamor_compile_glsl_prog(GL_VERTEX_SHADER,(char *)bt2020_pq_to_bt2020_linear_vertex_shader_glsl);

    priv->BT2020_pq_to_linear = glCreateProgram();
    glAttachShader(priv->BT2020_pq_to_linear, vs);
    glAttachShader(priv->BT2020_pq_to_linear, fs);
    glDeleteShader(vs);
    glDeleteShader(fs);

    glamor_link_glsl_prog(pScreen,priv->BT2020_pq_to_linear,"BT2020_pq_to_linear");

    glamor_program *prog =  &priv->BT2020_pq_to_linear_glamor;


    prog->locations = glamor_priv->copy_area_prog.locations;
    prog->prim_use = glamor_priv->copy_area_prog.prim_use;
    prog->prog = priv->BT2020_pq_to_linear;
    fill_glamor_prog_variables(prog);
...
void HDR_copy_find_hdr_glamor_program(ScreenPtr screen,glamor_screen_private *glamor_priv, DrawablePtr src,PixmapPtr src_backing,DrawablePtr dst,PixmapPtr dst_backing,glamor_program **prog)
{
  ...
  if (src_priv->purpose == HDR_pixmap_HDR && dst_priv->purpose == HDR_pixmap_intermiate){

      if (src_priv->colorspace == HDR_colorspace_BT2020){
          if (src_priv->tf == HDR_impl_tf_linear_norm_bt2020){
              *prog = &hdrpriv->BT2020_linearnorm_to_linear_glamor;
          } else if (src_priv->tf == HDR_impl_tf_pq){
              *prog = &hdrpriv->BT2020_pq_to_linear_glamor;
          }
      }
  }
...
void hdr_WindowUpdateSDRParams(ScreenPtr screen, WindowPtr pwindow, HDR_SDRPARAMS_uniform_t *sdrparams){
  ...
  /* XXX: redraw window, the content (backing texgure) didn't change, only fbo and output plane*/
