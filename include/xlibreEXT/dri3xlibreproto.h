...
typedef struct {
  ...
  CARD16 vendor_name_offset;         //! vender name offset starting from structure start beggining
  ...
  CARD32 pad;
  CARD64 flags;                    //! device usage flags
  CARD64 prefer;                   //! device preference flags
} xDRI3GetDeviceUserPreferencesGPUDevice;

#define sz_xDRI3GetDeviceUserPreferencesGPUDevice	56
#define szmin_xDRI3GetDeviceUserPreferencesGPUDevice (sz_xDRI3GetDeviceUserPreferencesGPUDevice + 16)

#define xDRI3GetDeviceUserPreferences_struct_flag_NOTDRM 1 //! drmMajor, drmMinor doens't contains /dev/dri/* node minor:major

#define xDRI3GetDeviceUserPreferences_flag_DESKTOPRENDER 1 //! There are no/little penalty to process windows pixmaps on this device (for example you can run compositor processing)
